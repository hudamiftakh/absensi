...
class absensi extends CI_Controller {
	...
	public function cron_send_wa_masuk(){
		...
		if($total_absensi>=1){
			...
			$text = "*NOTIFIKASI ABSEN MASUK MIN 1 JOMBANG* \n \n Yth. Bapak/Ibu *".$siswa['nama_ayah']."*  \n Siswa dengan NIS ".$dAbsensi['nis']." Atas nama *".$dAbsensi['nama']." Kelas ".$dAbsensi['kelas']."* Sudah melakukan absensi pada tanggal *".farmat_tanggal($dAbsensi['tanggal'])."* jam *".$dAbsensi['jam_masuk']."* dengan status *".$keterangan."* ".$menit." \n \n _Pesan otomatis sistem absensi MIN 1 Jombang_";
	...
	public function cron_send_wa_pulang(){
		...
		if($total_absensi>=1){
			...
			$text = "*NOTIFIKASI ABSEN PULANG MIN 1 JOMBANG* \n \n Yth. Bapak/Ibu *".$siswa['nama_ayah']."*  \n Siswa dengan NIS ".$dAbsensi['nis']." atas Nama *".$dAbsensi['nama']." Kelas ".$dAbsensi['kelas']."* Sudah melakukan absensi pada tanggal *".farmat_tanggal($dAbsensi['tanggal'])."* jam *".$dAbsensi['jam_pulang']."* dengan status *".$keterangan."* ".$menit." \n \n _Pesan otomatis sistem absensi MIN 1 Jombang_";
	...
	public function cron_notifikasi_walikelas(){
		...
		if($total_log_kelas>=1){
			...
			if($rest){
				...
				$this->db->update('tb_kelas',array('send_wa_status_masuk'=>'done', 'tgl_log_send_wa'=>date('Y-m-d H:i:s')));
			}
		}else{
			echo json_encode(array('status'=>'Data sudah dikirim semua'));
		}
